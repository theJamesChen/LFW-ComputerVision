...
def test(testfile, loadPath, gpu):
	...
	for batch_idx, data in enumerate(testing_dataloader):
		...
		if gpu:
			image1, image2, label = image1.cuda(), image2.cuda(), label.cuda() # On GPU
		image1, image2 = Variable(image1.float(), volatile=True), Variable(image2.float(), volatile=True)
		image1out, image2out = model(image1,image2)
		#loss = criterion(torch.squeeze(image1out), torch.squeeze(image2out), label)
		pdist = nn.PairwiseDistance(p=2)
		...
		histogram.append(', '.join(map(str,np.squeeze(np.transpose(euclideanDistance.data.cpu().numpy())))))
		thresh = 0.75
		if gpu:
...
def savePlot(iteration_history, loss_history, text):
	...
	title = "p1bLossVsIterationFor " + text[0] + text[1] + text[2]
	plt.title(title)
	plt.ylabel('Loss')
	plt.xlabel('Iterations')
	#plt.show()
	savetitle = "p1bLossIteration" + text[0] + text[1] + text[2]
...
def parse_args():
	...
	parser.add_argument('--margin', type=float, default=1.0, help='Sets margin term that nudges the network to separate features even more. Default is 1.5')
